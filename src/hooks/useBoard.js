...
export const useBoard = (boardId) => {
    ...
    // ------------------- Sync offline queue -------------------
    useEffect(() => {
        if (!offlineQueue.length || isOffline || syncing)
            return;
        const syncQueue = async () => {
            setSyncing(true);
            setSyncFailed(false);
            try {
                const results = await Promise.all(offlineQueue.map(op => {
                    if (op.type === "create")
                        return api.createTask(boardId, op.data);
                    if (op.type === "update" && op.taskId)
                        return api.updateTask(boardId, op.taskId, op.data);
    ...
    const createTask = useMutation({
        ...
        onSuccess: (newTask) => {
            queryClient.setQueryData(["board", boardId], oldBoard => {
                ...
                const newColumns = oldBoard.columns.map(col => col.id === newTask.columnId
